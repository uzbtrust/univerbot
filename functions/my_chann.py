...
def _build_channel_keyboard(channel_id: int, is_premium: bool = False):
    ...
    for i in range(1, max_posts + 1):
        if is_premium:
            # premium_channel: user_id(0), id(1), post1(2), theme1(3), ...
            post_idx = 2 + (i - 1) * 2
            theme_idx = 3 + (i - 1) * 2
        else:
            # channel: user_id(0), id(1), post1(2), theme1(3), ...
            post_idx = 2 + (i - 1) * 2
            theme_idx = 3 + (i - 1) * 2
    ...
    for i in posts_found[:3]:  # Max 3 posts in display
        keyboard.append([
        ...
        if is_premium:
            # Add image toggle for premium
            keyboard.append([
                InlineKeyboardButton(
                    text=f"🖼️ {i}-post rasm",
...
async def is_premium(call: CallbackQuery):
    """Check if user is premium and show appropriate channel list."""
...
async def change_time(call: CallbackQuery, state: FSMContext):
    """Handle free channel time change request."""
    try:
        try:
...
async def process_new_time(message: Message, state: FSMContext):
    """Process new time for free channel post."""
    try:
...
async def change_theme(call: CallbackQuery, state: FSMContext):
    """Handle free channel theme change request."""
    try:
        try:
            await call.message.delete()
        except Exception:
...
async def process_new_theme(message: Message, state: FSMContext):
    """Process new theme for free channel post."""
    try:
        data = await state.get_data()
        channel_id = data.get("channel_id")
        post_num = int(data.get("post_num"))
...
async def change_premium_time(call: CallbackQuery, state: FSMContext):
    """Handle premium channel time change request."""
    try:
        try:
            await call.message.delete()
        except Exception:
...
async def process_new_premium_time(message: Message, state: FSMContext):
    """Process new time for premium channel post."""
    try:
        data = await state.get_data()
        channel_id = data.get("channel_id")
        post_num = int(data.get("post_num"))
...
async def change_premium_theme(call: CallbackQuery, state: FSMContext):
    """Handle premium channel theme change request."""
    try:
        try:
            await call.message.delete()
        except Exception:
...
async def process_new_premium_theme(message: Message, state: FSMContext):
    """Process new theme for premium channel post."""
    try:
        data = await state.get_data()
        channel_id = data.get("channel_id")
        post_num = int(data.get("post_num"))
...
async def toggle_image_premium(call: CallbackQuery):
    """Toggle image generation for a specific premium post."""
    try:
        try:
            await call.message.delete()
        except Exception:
        ...
        # Get current image status (image columns start after post/theme pairs)
        # premium_channel structure: user_id(0), id(1), post1(2), theme1(3), ..., post15(32), theme15(33), image1(34), ...
        ...
        # Update database using direct query
        ...
        if channel:
            ...
            await call.message.answer(
                f"🖼️ {post_num}-post uchun rasm {status_text}\n\n{response}",
                reply_markup=keyboard,
                parse_mode="HTML"
            )
        else:
