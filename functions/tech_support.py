"""
Technical support module for premium users.
"""
import logging
...
async def request_support(call: CallbackQuery, state: FSMContext):
    """
    Handle technical support request from premium users.
    
    Args:
        call: Callback query from user
        state: FSM context
    """
    try:
...
async def process_support_message(message: Message, state: FSMContext, bot: Bot):
    """
    Process technical support message and forward to admin.
